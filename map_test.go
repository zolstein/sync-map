...
// mapOps is defined in version-specific test files.
...
// mapCall.apply is defined in version-specific test files.
...
func applyCalls(m casMapInterface, calls []mapCall) (results []mapResult, final map[any]any) {
	for _, c := range calls {
		v, ok := c.apply(m)
...
func TestConcurrentRange(t *testing.T) {
	...
	m := new(sync_map.Map[int64, int64])
...
func TestMapRangeNestedCall(t *testing.T) { // Issue 46399
	...
	if length != 0 {
		t.Fatalf("Unexpected sync_map.Map size, got %v want %v", length, 0)
	}
}
